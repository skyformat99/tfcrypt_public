...
void base64_eprint(FILE *where, struct base64_encodestate *estate, const char *input, size_t inputl)
{
	char tmp[256];
	ssize_t ix = inputl;
	...
	while (ix > 0) {
		...
		estate->count = 0;
		base64_encode_block(input, ix > 128 ? 128 : ix, tmp, estate);
		ix -= 128;
		if (ix < 128) base64_encode_blockend(tmp, estate);
		fprintf(where, "%s", tmp);
		fflush(where);
	}
